...
// Implement `Ord` so that the "maximum" frontier node is the one with
// the smallest distance. This makes it possible to use a BinaryHeap
// as min-priority queue for Dijkstra's algorithm.
